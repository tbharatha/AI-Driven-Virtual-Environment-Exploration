...
def plot_training_metrics(train_losses, avg_rewards, avg_qs, eval_rewards, eval_qs, evaluation_interval):
...
if __name__ == '__main__':
    ...
    # total_reward = 0
    # total_Q = 0
    num_episodes = 10  # Number of episodes to average over
    ...
    if args.evaluate_only:
        ...
        while num_steps < args.max_num_steps:
            ...
            if num_steps >= args.learn_start:
                ...
                if num_steps % args.replay_frequency == 0:
                    # loss = dqn.compute_loss(mem, args.batch_size)
                    # train_losses.append(loss.item())  # Append loss to train_losses
            ...
            if num_steps % args.log_interval == 0:
                ...
                avg_qs.append(avg_q)
            # # Calculate loss and append to train_losses
            # if num_steps % args.log_interval == 0:
            #     loss = dqn.compute_loss(mem)
            #     train_losses.append(loss.item())

            # # Calculate average reward per episode and append to avg_rewards
            # if done:
            #     avg_rewards.append(env.episode_reward)
            #     env.episode_reward = 0

            # # Calculate average Q-value per step and append to avg_qs
            # if num_steps % args.log_interval == 0:
            #     avg_q = np.mean(dqn.compute_q_values(state).detach().cpu().numpy())
            #     avg_qs.append(avg_q)

                # # Save checkpoints
                # if num_steps % checkpoint_interval == 0:
                #     dqn.save_model(filename='checkpoint_{}.pth.tar'.format(num_steps))
        ...
        plot_training_metrics(train_losses, avg_rewards, avg_qs, eval_rewards, eval_qs, int(args.evaluation_interval))
