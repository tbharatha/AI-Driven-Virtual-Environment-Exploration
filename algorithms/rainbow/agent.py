...
class Agent:
    ...
    def save_model(self, filename='model.pth', avg_rewards=None, avg_Q_values=None):
        # Save model parameters to a file
        torch.save(self.online_net.state_dict(), filename)
        if avg_rewards is not None:
            np.save(filename.replace('.pth', '_avg_rewards.npy'), avg_rewards)
        if avg_Q_values is not None:
            np.save(filename.replace('.pth', '_avg_Q_values.npy'), avg_Q_values)

    def load_model(self, filename='model.pth', avg_rewards=None, avg_Q_values=None):
        # Load model parameters from a file
        self.online_net.load_state_dict(torch.load(filename))
        return avg_rewards, avg_Q_values

    def compute_loss(self, mem, batch_size):
        tree_idxs, states, actions, returns, next_states, nonterminals, weights = mem.sample(batch_size)

        # Forward pass to compute Q-values for current and next states
        current_q_values = self.online_net(states).gather(1, actions.unsqueeze(1)).squeeze(1)
        next_q_values = self.target_net(next_states).max(1)[0].detach()

        # Compute expected Q-values using double Q-learning
        expected_q_values = returns + self.discount * nonterminals * next_q_values

        # Compute TD errors and weighted loss
        td_errors = expected_q_values - current_q_values
        loss = (F.smooth_l1_loss(current_q_values, expected_q_values, reduction='none') * weights).mean()

        # Update priorities in memory
        mem.update_priorities(tree_idxs, td_errors.abs().cpu().numpy() + 1e-6)
